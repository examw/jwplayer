package com.longtailvideo.jwplayer.view.components
{
	...
	import com.longtailvideo.jwplayer.view.RightclickMenu;
	...
	public class DisplayComponent extends CoreComponent implements IDisplayComponent
	{
		...
		//双击状态布尔值
		protected var _DouClickState:Boolean = false;
		protected var _playerr:Player = new Player();
		protected var _click:MouseEvent = new MouseEvent(MouseEvent.CLICK);
		...
		public function DisplayComponent(player:IPlayer)
		{
			...
			setupDisplayObjects();
			setupIcons();
			if (! isNaN(getConfigParam('bufferrotation')))
			{
				_bufferRotationAngle = Number(getConfigParam('bufferrotation'));
			}
			if (! isNaN(getConfigParam('bufferinterval')))
			{
				_bufferRotationTime = Number(getConfigParam('bufferinterval'));
			}
		...
		{
			//ApexTrace.trace(this.name);
			//设定检测阈值;
			setTimeout(clearDouClick, 200);
			//移除自己
			this.removeEventListener(MouseEvent.CLICK, handler_Click);
			//添加双击检测;
			this.addEventListener(MouseEvent.CLICK, handler_DouClickCheck);
		}

		private function handler_DouClickCheck($e:MouseEvent):void
		{
			_DouClickState = true;
			//只能写在这里，写在双击处理里面没用，日了
			processDouClick();
		}

		private function clearDouClick():void
		{

			if (_DouClickState)
			{
				//双击处理
				ApexTrace.trace("双击显示区域");
				_DouClickState = false;
				/*try
				{
					processDouClick();
				}
			...
			this.addEventListener(MouseEvent.CLICK, handler_Click);
			//移除双击检测侦听
			this.removeEventListener(MouseEvent.CLICK, handler_DouClickCheck);
		}

		private function processDouClick():void
		{
			//instance99这个名字在每个版本里面不同，请用monsterDebuger查看,加了皮肤，里面的对象列表名称变了，就不能用这种模拟的方法了。
			/*target = this.parent.getChildByName("instance99") as MovieClip;
			target = target.getChildByName("controlbar") as MovieClip;
			target.getChildByName("fullscreenButton").dispatchEvent(_click);*/
		...
		protected function updateRotation():void
		{
			if (_bufferIcon)
			{
				_bufferIcon.rotation +=  _bufferRotationAngle;
			}
		}
